    ...
    public void updateConnectedServer(List<String> allServerAddress) {
        if (allServerAddress != null) {
            ...
            if (allServerAddress.size() > 0) {
                ...
                //Add new server node
                //让客户端与新的 RpcServer 地址建立长连接（对于已经建立了连接的地址来说，就不再建立新连接），
                //并且把连接中的 RpcClientHandler 添加到 connectedHandlers，然后把
                //服务器的地址与 RpcClientHandler 作为 Key -> Value 键值对，一起添加到 connectedServerNodes 中
                ...
                //由于可能有些服务器更换地址或者说掉线，所以从 connectedHandlers 中移除掉对应连接中的 handler
                ...
                //如果从 Zookeeper 注册中心中没有发现服务器地址，那么关闭 connectedServerNodes 中所有的连接并且清空 connectedHandlers
            ...
            channelFuture.addListener(new ChannelFutureListener() {
                ...
                public void operationComplete(final ChannelFuture channelFuture) throws Exception {
                    if (channelFuture.isSuccess()) {
                        logger.debug("Successfully connect to remote server. the remote peer = " + remotePeer);
